{
  ...
  {
   for (j in 1:length(monthvector)) 
   {
     if(weatherData$month[i] == monthvector[j])
     {
  ...
  heatCoolByMonth2 = data.frame(monthvector, cooldays, heatdays);
  ...
  variable = rep("cooldays", 12);
  coolByMonth = data.frame(monthvector,  variable, cooldays);
  variable = rep("heatdays", 12);
  heatByMonth = data.frame(monthvector, variable, heatdays);
  #issue with  this append
  meltedDF2 = rbind(coolByMonth, heatCoolByMonth);
