{
  ...
  noprecipvalues = weatherData$relHum[weatherData$precipitation==0];
  
  # 2) get the mean, median, and standard deviation for the data from #1
  
  centralTendenciesNoPrecip = c(mean(noprecipvalues),
                                median(noprecipvalues),
                                sd(noprecipvalues)
                                ); 
  ...
  noprecipquantile = quantile(noprecipvalues, probs = c(0.25, 0.75));
  
  # 4) solve for interquartile range (IQR -- the box height): 
  #    IQR = 3rd quartile - 1st quartile
  
  IQRnoprecip = noprecipquantile[2] - noprecipquantile[1];
  
  # 5) solve for extreme whisker ends: 
  #    - high: (3rd quartile) + 1.5*IQR
  #    - low: (1st quartile) - 1.5*IQR
  
  whiskerEndsnoPrecip = c(noprecipquantile[2] + 1.5*IQRnoprecip,
                          noprecipquantile[1] - 1.5*IQRnoprecip);  
  ...
  noprecipvalueshigh = noprecipvalues[noprecipvalues <= whiskerEndsnoPrecip[2] &
                                        noprecipvalues > noprecipquantile[2] ];
  noprecipvalueslow = noprecipvalues[noprecipvalues >= whiskerEndsnoPrecip[1] &
                                       noprecipvalues < noprecipquantile[1]  ];
  
  actualwhiskers = c(max(noprecipvalueshigh),
                     min(noprecipvalueslow));
  
  
#     weatherDataNoOutliers = rep(0, 366);
#   
# for (day in 1:length(noprecipvalues)) 
# {
#   if(noprecipvalues[day] >= whiskerEndsnoPrecip[2] 
#      & noprecipvalues[day] <= whiskerEndsnoPrecip[1])
#   {
#     weatherDataNoOutliers[day] = noprecipvalues[day];
#   }  
#   else
#   {
#     weatherDataNoOutliers[day] = "NA";
#   }
# }  
# 
# actualwhiskersNoPrecip = c(max(weatherDataNoOutliers),
#                            min(weatherDataNoOutliers));
