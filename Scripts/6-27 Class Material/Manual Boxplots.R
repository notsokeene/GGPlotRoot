{
  ...
  noPrecipDays = weatherData$relHum[weatherData$precipitation==0];
  
  # 2) get the mean, median, and standard deviation for the data from #1
  
  precipstats = c(mean(noPrecipDays),
                  median(noPrecipDays),
                  sd(noPrecipDays));
  ...
  relHumQuant = quantile(noPrecipDays, probs = c(.25, .75));
  
  
  # 4) solve for interquartile range (IQR -- the box height): 
  #    IQR = 3rd quartile - 1st quartile
  
  relHumIQR = relHumQuant[2] - relHumQuant[1];
  
  # 5) solve for extreme whisker ends: 
  #    - high: (3rd quartile) + 1.5*IQR
  #    - low: (1st quartile) - 1.5*IQR
  
  whiskerEnds = c(relHumQuant[2] + 1.5*relHumIQR,
                  relHumQuant[1] - 1.5*relHumIQR);
